...
def run(dir_in, cal_dir=None, bpr=True):
    ...
    for bursti, npims in enumerate(bursts):
        ...
        print("% 5u   avg min: % 5u, median: % 5u, max: % 5u      RMS min %0.6f med %0.6f max %0.6f" % (bursti,
                                                                                                        np.ndarray.min(np.array(npim_avg)), np.median(npim_avg), np.ndarray.max(np.array(npim_avg)),
                                                                                                        np.ndarray.min(npim_rms), median_rms, np.ndarray.max(npim_rms)))
        if 0:
            plt.clf()
